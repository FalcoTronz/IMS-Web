...
$cacheFile = sys_get_temp_dir()."/borrowings_trend_{$days}.json"; $ttl = 120;

if (file_exists($cacheFile) && (time()-filemtime($cacheFile) < $ttl)) { readfile($cacheFile); exit; }

$ch = curl_init($apiBase."/borrowings-trend?days=".$days);
curl_setopt_array($ch, [CURLOPT_RETURNTRANSFER=>true, CURLOPT_HTTPHEADER=>['X-API-KEY: '.$apiKey], CURLOPT_TIMEOUT=>20]);
$res = curl_exec($ch); $code = curl_getinfo($ch, CURLINFO_HTTP_CODE); curl_close($ch);

if ($code !== 200 || !$res) { http_response_code(502); echo json_encode(["error"=>"Upstream error from /borrowings-trend"]); exit; }
